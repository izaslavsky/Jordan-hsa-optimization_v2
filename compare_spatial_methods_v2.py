...
def main():
    ...
    parser.add_argument("--out-dir",
                        default=os.environ.get("HSA_OUT_DIR",
                                               os.environ.get("PIPELINE_OUT_DIR", "out")),
                        help="Run output directory (default: $HSA_OUT_DIR, else out)")
    parser.add_argument(
        "--output-csv",
        default=None,
        help="Path for comparison output CSV (default: <out-dir>/{network}_{hsa_mode}_spatial_methods_comparison.csv)",
    )
    ...
    # Save
    # Resolved from the run directory, not the literal "out": each run writes to
    # its own directory, and a hardcoded path drops this run's comparison on top
    # of another run's file.
    _out_root = Path(args.out_dir)
    output_csv = args.output_csv or str(_out_root / f"{network}_{mode}_spatial_methods_comparison.csv")
